...
export interface EfConfigurationOptions {
...
export const PRESET_CUSTOMER_ORDERS = `public class Customer
{
...
export const PRESET_BLOG_POST = `public class Blog
{
    public int Id { get; set; }
...
export function parseCSharpEntities(
  source: string,
  options: { tableNaming?: string; schema?: string } = {},
): { entities: EntityInfo[]; enums: string[] } {
  ...
  const defaultSchema = options.schema?.trim() || '';
  ...
  while ((match = classRegex.exec(source)) !== null) {
    ...
    const tableAttrMatch = rawAttrs.match(
      /Table\s*\(\s*"([^"]+)"(?:\s*,\s*Schema\s*=\s*"([^"]+)")?\s*\)/,
    );
    if (tableAttrMatch) {
    ...
    const propRegex =
      /(?:\[([^\]]+)\]\s*)*(?:public|protected|internal)?\s*(?:virtual\s+|override\s+|required\s+)*([\w<>?,.\[\]]+)\s+(\w+)\s*\{\s*get;\s*(?:set|init)?;\s*\}(?:\s*=\s*[^;]+;)?/g;
    ...
    while ((propMatch = propRegex.exec(body)) !== null) {
      ...
      const colMatch = cleanType.match(
        /^(?:ICollection|IList|List|HashSet|IEnumerable|ISet)<(\w+)>$/,
      );
    ...
    properties.forEach((p) => {
      if (!p.isKey && !p.isCollection && !p.isForeignKey) {
        const isPrimitive =
          /^(int|long|short|byte|guid|string|bool|datetime|datetimeoffset|dateonly|timeonly|decimal|double|float|char|byte\[\])$/i.test(
            p.type,
          );
...
export function generateEfConfiguration(
  source: string,
  options: EfConfigurationOptions = {},
  mode: 'fluent' | 'annotations' | 'both' = 'fluent',
): string {
  const { entities, enums } = parseCSharpEntities(source, options);

  if (entities.length === 0) {
    return '// No C# class or record definitions found.\n// Please paste one or more C# entity classes above.';
  }

  const defaultStringLength =
    options.defaultStringLength !== undefined ? Number(options.defaultStringLength) : 200;
  const deleteBehavior = options.deleteBehavior || 'Restrict';
  const enableSoftDelete = options.enableSoftDelete ?? true;
  const enablePrecision = options.enablePrecision ?? true;
  const enableEnumConversion = options.enableEnumConversion ?? true;
    ...
    const fluentOutputs = entities.map((entity) => {
      ...
      entity.properties.forEach((prop) => {
        ...
        if (typeLower === 'string') {
          ...
          if (
            !prop.nullable &&
            (typeLower === 'guid' || typeLower === 'datetime' || typeLower === 'datetimeoffset')
          ) {
            chain.push('.IsRequired()');
          }
        }
      ...
      const indexProps = entity.properties.filter(
        (p) =>
      ...
      if (
        ...
        entity.navigations.length > 0 ||
        entity.collections.length > 0
      ) {
        lines.push(`        // Relationships & Foreign Keys`);
        entity.navigations.forEach((nav) => {
          const matchingFk = entity.foreignKeys.find(
            (fk) => fk.foreignKeyFor?.toLowerCase() === nav.name.toLowerCase(),
          );
          const fkName = matchingFk ? matchingFk.name : `${nav.name}Id`;
          const matchingEntity = entities.find(
            (e) => e.name.toLowerCase() === nav.type.toLowerCase(),
          );
          const inverseCollection = matchingEntity?.collections.find(
            (c) => c.collectionItemType?.toLowerCase() === entity.name.toLowerCase(),
          );
        ...
        entity.collections.forEach((col) => {
          ...
          const inverseNav = targetEntity?.navigations.find(
            (n) => n.type.toLowerCase() === entity.name.toLowerCase(),
          );
      ...
      if (enableSoftDelete) {
        const softDeleteProp = entity.properties.find(
          (p) => /^isdeleted$/i.test(p.name) || /^isactive$/i.test(p.name),
        );
        if (softDeleteProp) {
  ...
  if (mode === 'annotations' || mode === 'both') {
    const annotOutputs = entities.map((entity) => {
      ...
      const indexProps = entity.properties.filter(
        (p) =>
          /email|username|slug|code|sku/i.test(p.name) &&
          !p.isKey &&
          !p.isCollection &&
          !p.isNavigation,
      );
